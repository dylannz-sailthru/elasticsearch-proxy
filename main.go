...
func main() {
	...
	envs := strings.Split(os.Getenv("AWS_ESPROXY_ENVIRONMENTS"), ",")
	wg := sync.WaitGroup{}
	for k, v := range envs {
		if v == "" {
			logrus.Fatal("Empty environment name, has AWS_ESPROXY_ENVIRONMENTS been configured correctly?")
		}

		listenPort := os.Getenv(fmt.Sprintf("AWS_ESPROXY_%s_LISTEN_PORT", v))
		...
		host := os.Getenv(fmt.Sprintf("AWS_ESPROXY_%s_HOST", v))
		if host == "" {
